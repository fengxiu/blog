---
title: java集合系列10之HashMap&HasSet源码分析
tags:
  - 集合
categories:
  - java
...
HashMap就是使用哈希表来存储的。哈希表为解决冲突，可以采用开放地址法和链地址法等来解决问题，其它的解决冲突的方法可以参考[解决哈希冲突的常用方法分析](https://cloud.tencent.com/developer/article/1672781)。Java中HashMap采用了链地址法。链地址法，简单来说，就是数组加链表的结合。在每个数组元素上都一个链表结构，当数据被Hash后，得到数组下标，把数据放在对应下标元素的链表上。
...
static int indexFor(int h, int length) {  // length对应着table的长度
 ...
 * key ：key值
 * value – value值
 * onlyIfAbsent – 如果为true，只有key不存在的时候才插入新值
 * evict – LinkedList会使用，标记是创建模式还是更新模式
 * 
 */
...
1. JDK1.7中，当两个线程同时进行插入操作时，同时执行到createEntry方法时，获取到了同一个头节点e，第二个线程会覆盖掉第一个线程的插入操作，使第一个线程插入的数据丢失。JDK1.8中的尾插法同样会有这样的问题，两个线程获取到相同的节点，然后把新键值对赋值给这个节点的next，后面的赋值操作覆盖掉前面的。
2. JDK1.7和JDK1.8中对map进行扩容时，由于节点的next会变化，造成实际有key值，但是读操作返回null的情况。
3. 1.7中，当两个线程同时进行扩容操作时，可能会造成链表的死循环
...
jdk1.7和1.8详细对比，[JAVA集合：HashMap深度解析（版本对比）](https://juejin.cn/post/6844903555149594638#heading-19)

1. 处理hash冲突，俩者都是采用链表地址法，jdk8会在此基础上，将链表长度大于8的时候，转换成红黑树。
2. 加入了jdk.map.althashing.threshold这个jdk的参数用来控制是否在扩容时使用String类型的新hash算法。
把1.6的构造方法中对表的初始化挪到了put方法中，1.6中的tranfer方法对旧表的节点进行置null操作（存在多线程问题），1.7中去掉了。

[HashMap面试题及答案（2022版）](https://www.cnblogs.com/java1024/p/13488714.html)
...
1. [美团技术博客](https://tech.meituan.com/2016/06/24/java-hashmap.html)