...
**而notify()和notifyAll()的作用则是唤醒当前对象上的等待线程**notify()是唤醒单个线程，而notifyAll()是唤醒所有的线程。
...
sleep()定义在Thread.java中。 sleep()的作用是让当前线程休眠，即当前线程会从**运行状态**进入到**休眠(阻塞)状态**。sleep()会指定休眠时间，线程休眠的时间会大于等于该休眠时间；在线程重新被唤醒时，它会由**阻塞状态**变成**就绪状态**，从而等待cpu的调度执行。
