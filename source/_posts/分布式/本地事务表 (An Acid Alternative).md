...
**消息队列的排序**
关于有序消息传递的简短旁注是相关的。消息系统提供了确保消息按接收顺序发送的能力。这可能需要昂贵的支持，而且往往是不必要的，事实上，有时会给人一种虚假的安全感。
这里提供的示例说明了如何放松消息排序，并最终提供数据库的一致视图。放松排序所需的开销是名义上的，在大多数情况下比在消息系统中强制执行排序要少得多。
此外，无论交互方式如何，Web应用程序在语义上都是事件驱动的系统。客户端请求以任意顺序到达系统。每个请求所需的处理时间各不相同。整个系统组件中的请求调度是不确定的，导致消息队列不确定。要求保留命令会给人一种虚假的安全感。简单的现实是，不确定的输入将导致不确定的输出。
**软状态/最终一致**
到目前为止，重点一直放在可用性交易的一致性上。硬币的另一面是理解软状态和最终一致性对应用程序设计的影响。
...
然而，如果我们从用户的角度考虑这一点，这种滞后可能不相关或甚至是已知的。接收用户和发送用户都不知道资产何时到达。如果发送和接收之间的延迟为几秒钟，那么直接就资产转让进行沟通的用户将看不到或肯定可以忍受。在这种情况下，系统行为被认为是一致的，用户可以接受，尽管我们依赖于软状态和实现中的最终一致性。
